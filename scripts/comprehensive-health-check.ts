...
async function checkMySQL() {
  ...
  try {
    ...
    const existingTables = (tables as any[]).map((t: any) => Object.values(t)[0]);
    const missingTables = keyTables.filter(t => !existingTables.includes(t));
    
    await connection.end();

    if (missingTables.length === 0) {
      addResult('MySQL Database', 'healthy', `Connected successfully with ${tableCount} tables`, { tables: tableCount });
    } else {
      addResult('MySQL Database', 'warning', `Connected but missing ${missingTables.length} key tables`, { missing: missingTables, total: tableCount });
    }
  } catch (error: any) {
...
async function checkQdrant() {
  ...
  try {
    ...
    if (collectionNames.includes(expectedCollection)) {
      const collectionInfo = await client.getCollection(expectedCollection);
...
async function checkServiceImplementations() {
  ...
  // Check if implementations directory exists (even if empty)
  try {
    ...
    addResult('Service Layer', 'warning', 'Service implementations directory empty or missing');
  }
}
