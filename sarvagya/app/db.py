...
class Base(DeclarativeBase):
    # C1 fix: isolate all Sarvagya tables into their own schema
    # to avoid collisions with Better Auth tables in the public schema.
    if _DB_SCHEMA:
        metadata = DeclarativeBase.metadata
        metadata.schema = _DB_SCHEMA
...
# C1 fix: set search_path so all DDL / queries target the sarvagya schema
_connect_args = {}
if _DB_SCHEMA:
    _connect_args["server_settings"] = {"search_path": _DB_SCHEMA}
