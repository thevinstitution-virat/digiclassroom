...
/**
 * Selfstudy Buddy Agent - Socratic Tutoring Implementation
 * Provides step-by-step guidance without giving direct answers
 */

import { BaseAgent, AgentRequest, AgentResponse, StreamingAgentResponse, CoreAgentServices, ConversationMessage } from './core/base-agent';
...
export class SocraticTutoringTool extends BaseAgent {
  ...
  constructor(capabilities?: AgentCapabilities, config?: AgentConfig, services?: CoreAgentServices) {
    ...
    logger.info('🎓 Socratic Tutoring Tool initialized with Adaptive Tutoring (Option C)');
  }
  ...
  private countConversationTurns(
    ...
    if (lastMessage.role === 'assistant' || lastMessage.role === 'agent') {
      ...
      if (isWelcomeMessage) {
        logger.info('👋 [Turn Count] Last message was welcome/greeting - treating current question as Turn 0');
        logger.info(`   Welcome message: "${lastMessage.content.substring(0, 60)}..."`);
        logger.info(`   Student\'s first question: "${currentQuestion.substring(0, 60)}..."`);
      ...
      logger.info('💬 [Turn Count] Student responding to agent\'s tutoring question - SAME TOPIC');
      logger.info(`   Agent asked: "${lastMessage.content.substring(0, 60)}..."`);
      logger.info(`   Student responds: "${currentQuestion.substring(0, 60)}..."`);
    ...
    if (this.isNewTopic(lastStudentMessage.content, currentQuestion)) {
      logger.info('🆕 [Turn Count] New topic detected - resetting to Turn 0');
      logger.info(`   Last question: "${lastStudentMessage.content.substring(0, 50)}..."`);
      logger.info(`   Current question: "${currentQuestion.substring(0, 50)}..."`);
      return 0; // New topic = Turn 0
    }

    // Same topic - count student messages as turns
    logger.info('🔄 [Turn Count] Same topic continuation');
    return studentMessages.length;
  }
  ...
  private isNewTopic(previousQuestion: string, currentQuestion: string): boolean {
    ...
    if (isNew) {
      logger.info(`📊 [Topic Detection] Similarity: ${(similarity * 100).toFixed(1)}% - NEW TOPIC`);
    } else {
      logger.info(`📊 [Topic Detection] Similarity: ${(similarity * 100).toFixed(1)}% - SAME TOPIC`);
    }
  ...
  private determineAdaptiveMode(
    ...
    if (turns === 0) {
      logger.info('🎓 [Adaptive] Turn 0 - Asking about prior knowledge (knowledge assessment)');
    ...
    if (this.detectExplicitRequest(request.student_question, conversationHistory)) {
      logger.info('🎯 [Adaptive] Explicit request detected - switching to EXPLANATION mode');
    ...
    if (this.detectFrustration(conversationHistory)) {
      logger.info('😓 [Adaptive] Frustration detected - switching to SCAFFOLDING mode');
      return { mode: 'scaffolding', reason: 'Student showing signs of frustration' };
    }

    // Rule 3: Conversation length threshold (3+ turns)
    if (turns >= 3) {
      logger.info('⏱️ [Adaptive] 3+ conversation turns - switching to EXPLANATION mode');
    ...
    if (knowledgeLevel === 'zero') {
      logger.info('📚 [Adaptive] Zero knowledge detected - switching to SCAFFOLDING mode');
      return { mode: 'scaffolding', reason: 'Student has zero prior knowledge' };
    }

    if (knowledgeLevel === 'minimal' && turns >= 1) {
      logger.info('📖 [Adaptive] Minimal knowledge + 1 turn - switching to EXPLANATION mode');
    ...
    logger.info('❓ [Adaptive] Using QUESTIONING mode (Socratic method)');
  ...
  async provide_socratic_guidance(request: SocraticGuidanceRequest): Promise<SocraticGuidanceResponse> {
    logger.info(`🎓 Adaptive Tutoring: ${request.student_question.substring(0, 50)}... for Class ${request.grade_level}`);
    ...
    try {
      ...
      const context = await this.services.retrieval.searchRelevantContent({
        query: request.student_question,
        grade_level: request.grade_level,
        subject: request.subject,
        board_type: request.board_type,
        limit: 5,
        content_types: ['concepts', 'explanations', 'examples']
      });
      ...
      // Step 6: Generate guidance using LLMProvider with TRUE multi-turn conversation
      ...
      logger.info(`💬 [Conversation] Sending ${messages.length} messages to LLM (including ${request.conversation_history?.length || 0} history messages)`);
      ...
      logger.info(`✅ Adaptive guidance generated [${adaptiveDecision.mode.toUpperCase()}]: ${verificationResult.overall_fidelity_score.toFixed(3)} fidelity`);
      logger.info(`   Reason: ${adaptiveDecision.reason}`);
  ...
  private formatSocraticConversationHistory(history: Array<{ role: string, content: string }>): string {
    if (history.length === 0) {
  ...
  private buildAdaptiveTutoringPrompt(
    ...
    const conversationHistory = this.formatSocraticConversationHistory(request.conversation_history || []);
...
- Being discouraging or negative about mistakes`;
  ...
  // ============================================================================
  // IAgent Interface Implementation (Adapters)
  // ============================================================================

  async execute(request: AgentRequest): Promise<AgentResponse> {
    const startTime = Date.now();
    const guidanceRequest: SocraticGuidanceRequest = {
    ...
    const guidance = await this.provide_socratic_guidance(guidanceRequest);
    ...
    return {
      ...
      metadata: {
        ...
        sources: guidance.citations || [],
        fidelity: guidance.fidelity_score || 0,
  ...
  async executeStreaming(request: AgentRequest): Promise<StreamingAgentResponse> {
    throw new Error('Streaming not implemented on legacy full agent yet. Pending new architecture.');
  }
}
...
export class HomeworkHelpAgent {
  ...
  async help_with_homework(
    ...
    logger.info(`📚 Homework Help Request: ${question.substring(0, 50)}... for Class ${studentContext.grade_level} ${studentContext.subject}`);
