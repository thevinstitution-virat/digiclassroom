...
function HealthWidget() {
  ...
  const { data, error, isLoading } = useSWR('/api/admin/engine/health', fetcher, { refreshInterval: 15000 })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        const sseUrl = `/api/admin/content/progress/${uploadId}`;
      ...
      if (result.success) {
        ...
        mutate('/api/admin/qdrant/books')
        mutate('/api/admin/qdrant/stats')
        console.log('🔄 SWR mutate triggered for /api/admin/qdrant/books and /api/admin/qdrant/stats')
      }
          ...
          <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl rounded-2xl p-8 shadow-xl border border-white/20 dark:border-gray-700/20 hover:shadow-2xl transition-all duration-300 transform hover:scale-[1.02]">
            ...
            <UploadProgressModal
              ...
              onCancel={async () => {
                ...
                await fetch('/api/admin/content/cancel', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ uploadId: activeUploadId }) })
