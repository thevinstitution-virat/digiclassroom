...
import QuestionImporter from '@/components/practest/admin/QuestionImporter'
import TestSeriesManager from '@/components/practest/admin/TestSeriesManager'
import { useBetterAuthUser } from '@/hooks/useBetterAuthUser'

type AdminView = 'overview' | 'questions' | 'editor' | 'import' | 'analytics' | 'settings'
...
export default function PractestAdminPage() {
  ...
  const loadAdminStats = async () => {
    try {
      ...
      const res = await fetch('/api/super-admin/practest/stats')
      ...
      if (data.success) {
        setState(prev => ({
          ...
          stats: {
            ...
            approvedQuestions: data.stats.approvedQuestions,
        ...
        setState(prev => ({ ...prev, error: data.error || 'Failed to load statistics', loading: false }))
      }
    } catch (error) {
  ...
  // Access control is enforced server-side by the /dashboard/super-admin layout
  // (super_admin only) — the old client-side `role === 'admin'` gate predated the
  // super_admin tier and wrongly denied the platform owner, so it's removed.
  return (
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      ...
      <Tabs value={state.currentView} onValueChange={(value) => handleViewChange(value as AdminView)}>
        <TabsList className="grid w-full grid-cols-3 sm:grid-cols-6">
          ...
          <TabsTrigger value="settings">Test Series</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="import">
          <QuestionImporter onImported={loadAdminStats} />
        </TabsContent>
