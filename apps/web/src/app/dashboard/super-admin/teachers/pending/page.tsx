...
export default function AdminPendingTeachersPage() {
  ...
  const fetchTeachers = async () => {
    try {
      const response = await fetch(`/api/super-admin/teachers/pending?status=${filter}`)
  ...
  const handleApprove = async (teacherId: string) => {
    ...
    try {
      const response = await fetch(`/api/super-admin/teachers/approve/${teacherId}`, {
        method: 'POST',
      })
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await fetch(`/api/super-admin/teachers/reject/${selectedTeacher.userId}`, {
        method: 'POST',
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <CardTitle className="text-lg">
                      {teacher?.name?.split(' ')[0]} {teacher?.name?.split(' ').slice(1).join(' ')}
                    </CardTitle>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-md">
            <CardHeader>
              ...
              <CardDescription>
                Provide a reason for rejecting {selectedTeacher?.name?.split(' ')[0]} {selectedTeacher?.name?.split(' ').slice(1).join(' ')}'s application
              </CardDescription>
            </CardHeader>
            <CardContent>
