...
function sanitizeMarkdown(content: string): string {
  ...
  // Wrap Key Terms section in a special div for styling
  // This allows us to apply special CSS to the entire section
  // IMPROVED: More flexible regex that handles different emojis, spacing, and separators
  // Matches: "---" or "***" or "___" followed by "### Key Terms" (with optional emoji)
  const keyTermsPattern = /(?:---|___|\*\*\*)\s*\n\s*###\s*[📚🎓📖🔑]?\s*Key\s+Terms[\s\S]*$/i
  const keyTermsMatch = sanitized.match(keyTermsPattern)

  // Debug logging (only in development)
  if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
    console.log('📋 KEY TERMS DETECTION DEBUG:')
    console.log('━'.repeat(60))
    console.log('- Found key terms:', keyTermsMatch !== null)
    if (keyTermsMatch) {
      console.log('- Match index:', keyTermsMatch.index)
      console.log('- Match length:', keyTermsMatch[0].length)
      console.log('- Full matched section (first 500 chars):')
      console.log(keyTermsMatch[0].substring(0, 500))
      console.log('- Newline count in match:', (keyTermsMatch[0].match(/\n/g) || []).length)
      console.log('- Double newline count:', (keyTermsMatch[0].match(/\n\n/g) || []).length)

      // Show structure
      const lines = keyTermsMatch[0].split('\n').slice(0, 20)
      console.log('- First 20 lines of key terms section:')
      lines.forEach((line, i) => {
        console.log(`  ${i}: "${line}" ${line === '' ? '(BLANK)' : ''}`)
      })
    } else {
      // Show what we're looking for
      const hasKeyTermsText = sanitized.includes('Key Terms')
      console.log('- Contains "Key Terms" text:', hasKeyTermsText)
      if (hasKeyTermsText) {
        const idx = sanitized.indexOf('Key Terms')
        console.log('- Context around "Key Terms" (100 chars before, 300 after):')
        console.log(sanitized.substring(Math.max(0, idx - 100), idx + 300))
      }

      // Check for separator
      const hasSeparator = sanitized.includes('---') || sanitized.includes('***') || sanitized.includes('___')
      console.log('- Contains separator (---, ***, ___):', hasSeparator)
    }
    console.log('━'.repeat(60))
  }

  if (keyTermsMatch) {
    const keyTermsSection = keyTermsMatch[0]
    const beforeKeyTerms = sanitized.substring(0, keyTermsMatch.index)

    // Wrap the key terms section with proper spacing
    // CRITICAL: Preserve all blank lines within the section
    sanitized = beforeKeyTerms + '\n\n<div class="key-terms-section">\n\n' + keyTermsSection + '\n\n</div>'

    if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
      console.log('✅ Key Terms section wrapped in div')
    }
  } else {
    if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
      console.warn('⚠️ Key Terms section NOT detected - CSS styling will not be applied!')
    }
  }
