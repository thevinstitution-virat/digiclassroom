...
const VERDICT_STYLES: Record<string, { bg: string; text: string; label: string }> = {
  insufficient_data: { bg: 'bg-muted', text: 'text-muted-foreground', label: '⏳ Insufficient Data' },
...
function MetricCard({ label, value, unit, delta }: { label: string; value: number | null; unit?: string; delta?: number | null }) {
  return (
    <div className="rounded-lg border border-border p-4 bg-card">
      ...
      {delta != null && (
        <div className={`text-xs mt-1 font-medium ${delta > 0 ? 'text-green-600' : delta < 0 ? 'text-red-600' : 'text-muted-foreground'}`}>
...
function ProviderPanel({ stats, isCanary }: { stats: ProviderStats; isCanary?: boolean }) {
  ...
  return (
    <div className={`rounded-xl border-2 p-6 ${isCanary ? 'border-blue-400 dark:border-blue-600' : 'border-input'} bg-card`}>
      <div className="flex items-center gap-2 mb-4">
        <span className={`w-3 h-3 rounded-full ${isCanary ? 'bg-blue-500' : 'bg-gray-400'}`} />
        <h3 className="text-lg font-semibold text-foreground">{providerLabel}</h3>
      ...
      <div className="text-sm text-muted-foreground mb-4">{stats.totalRequests} requests • {stats.ragas.sampleSize} with RAGAS</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground">🐤 Canary Comparison</h2>
          <p className="text-sm text-muted-foreground mt-1">{data.totalMessages} messages over {data.windowDays} days</p>
        </div>
        <div className="flex items-center gap-3">
          <select value={days} onChange={e => setDays(+e.target.value)} className="rounded-lg border border-input bg-card px-3 py-1.5 text-sm">
      ...
      <div className={`rounded-xl p-5 ${vs.bg}`}>
        ...
        <div className="mt-2 grid grid-cols-3 gap-4 text-sm">
          <div><span className="text-muted-foreground">RAGAS Δ (faith):</span>{' '}<span className="font-mono font-bold">{gates.ragasDelta?.faithfulness?.toFixed(4) ?? '—'}</span></div>
          <div><span className="text-muted-foreground">Latency Δ (p50):</span>{' '}<span className="font-mono font-bold">{gates.latencyDelta?.p50Ms != null ? `${gates.latencyDelta.p50Ms > 0 ? '+' : ''}${gates.latencyDelta.p50Ms}ms` : '—'}</span></div>
          <div><span className="text-muted-foreground">Cost ratio:</span>{' '}<span className="font-mono font-bold">{gates.costRatio != null ? `${gates.costRatio}x` : '—'}</span></div>
        </div>
      </div>
