...
export async function GET(_req: NextRequest) {
  ...
  // Pin to the CUDA-enabled Python 3.11 venv — system Python cannot import PDF-Extract-Kit.
  const pythonPath = process.env.DOC_EXTRACT_ENGINE_PYTHON_PATH || path.join(process.cwd(), '.venv-py311', 'Scripts', 'python.exe')
  ...
  try {
    // Verify the real extraction stack: PDF-Extract-Kit tasks import AND CUDA is available.
    await execFileAsync(pythonPath, ['-c', 'import sys, os; sys.path.insert(0, os.path.join(os.getcwd(), "vendor", "PDF-Extract-Kit")); import pdf_extract_kit.tasks; import torch; assert torch.cuda.is_available(); print("ok")'])
